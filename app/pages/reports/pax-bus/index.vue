<template>
  <div class="space-y-6 h-[78vh]">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-linear-to-br from-blue-600 via-indigo-600 to-purple-600 p-8 text-white shadow-2xl">
      ...
      <div class="relative z-10">
        <div class="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 class="text-4xl font-bold">Rapports Pax Bus</h1>
            <p class="text-white/90 text-sm mt-1">
              Statistiques des passagers transportés par bus
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="grid gap-6 md:grid-cols-2">
      <!-- Rapport Mensuel -->
      <Card class="group hover:shadow-xl transition-all border-2 hover:border-teal-500/50 cursor-pointer" @click="openMonthlyDialog">
        <CardHeader>
          ...
          <CardTitle class="mt-4">Rapport Mensuel Pax Bus</CardTitle>
          <CardDescription>
            Générer le rapport des passagers bus pour un mois spécifique
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card class="group hover:shadow-xl transition-all border-2 hover:border-teal-500/50 cursor-pointer" @click="openAnnualDialog">
        <CardHeader>
          ...
          <CardTitle class="mt-4">Rapport Annuel Pax Bus</CardTitle>
          <CardDescription>
            Générer le rapport des passagers bus pour une année complète
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button class="w-full" variant="outline">
    ...
    <Dialog v-model:open="monthlyDialogOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Rapport Mensuel Pax Bus</DialogTitle>
          <DialogDescription>
    ...
    <Dialog v-model:open="annualDialogOpen">
      <DialogContent class="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Rapport Annuel Pax Bus</DialogTitle>
          <DialogDescription>
...
const generateMonthlyReport = async () => {
  ...
  try {
    // TODO: Implémenter l'endpoint pour le rapport mensuel pax bus
    const url = `/pax-bus-report/export/${monthlyForm.value.month}/${monthlyForm.value.year}`
    ...
    link.download = `PAX_BUS_${monthlyForm.value.month}-${monthlyForm.value.year}.xlsx`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(downloadUrl)

    success('Rapport généré', 'Le rapport mensuel Pax Bus a été téléchargé avec succès')
...
const generateAnnualReport = async () => {
  ...
  try {
    // TODO: Implémenter l'endpoint pour le rapport annuel pax bus
    const url = `/pax-bus-report/export/${annualForm.value.year}`
    ...
    const link = document.createElement('a')
    link.href = downloadUrl
    link.download = `PAX_BUS_ANNUEL_${annualForm.value.year}.xlsx`
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
    window.URL.revokeObjectURL(downloadUrl)

    success('Rapport généré', 'Le rapport annuel Pax Bus a été téléchargé avec succès')
